function result = kfold_cross_validation_ORA_uncertainty()
    ...
    condition = 0;
            for recip = unique(data.recip)'
                for subj = subjvec'
                    ...
                    trial_nrs = cumsum(decisions(1:length(selection.choice))); % cumulative sum starts at 0
...
function probs = validate(data, pars_all)
    ...
    for subjIndex = 1:size(pars_all, 1)
        ...
        betaPrior  = pars(4);
                ...
                % Log likelihood
                subjProbs = [subjProbs; 1./(1 + exp(- thischoice .* (beta1 * (DeltaQ_vectorized(linearidx)))))];
            end
        end
